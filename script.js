...
const questions = [
    ...
    function startQuiz(){
      ...
      }else alert('Promise not checked');
    }
  ...
  // }];
    ...
    function caricamentoPaginaRisultato(){
      ...
      for(let i=0;i<questions.length;i++){
        ...
        if(answerTot[i].point==1) rightWrong=`<i class="fas fa-check-square" style="color:green;"></i>`;
        nodoQ.innerHTML += `<li class="riga">${rightWrong}${questions[i].question}<button class="showMore" value="${i}" > v </button></li>`;
      }
      let pin=0;
